...
[role="lead"]
Keystone STaaS offre livelli di servizio di prestazioni (PSL) predefiniti, anche chiamati livelli di servizio di prestazioni di base, per ogni tipo di storage. Ogni PSL specifica gli obiettivi di prestazioni (IOPS, throughput e latenza), i protocolli supportati, la piattaforma e i requisiti minimi di capacità. Esamina i PSL disponibili per scegliere il livello di servizio giusto per il tuo carico di lavoro e pianificare il tuo abbonamento.
