...
È possibile visualizzare l'utilizzo previsto della capacità per i prossimi 120 giorni a partire dalla data odierna tramite l'interruttore *Previsione* disponibile nella vista *Consumo storico* della scheda *Andamento dei consumi* nella Console. L'interruttore è abilitato per impostazione predefinita.

Di default, il grafico visualizza gli ultimi 30 giorni di consumo storico e una proiezione a 120 giorni dalla data odierna, sullo stesso grafico. Vengono tracciati fino a 30 punti dati in totale (20 per dati storici e 10 per dati di previsione), con una linea verticale che separa i due segmenti.

image:forecast-1.png["screenshot che mostra i dati di previsione"]

Selezionando un intervallo di date che non termina con la data odierna, le previsioni vengono automaticamente disabilitate. Per riattivarle, viene visualizzata una finestra di dialogo di conferma che informa che i dati di forecast sono disponibili solo quando la timeline selezionata termina con la data odierna. Confermando, la visualizzazione viene ripristinata all'intervallo predefinito degli ultimi 30 giorni di dati storici più i successivi 120 giorni di forecast.

Se si prevede che il consumo superi la capacità di picco, sul grafico compare un indicatore di allarme che consiglia un aumento di capacità.
...
Nella Console, oltre all'utilizzo burst fatturato, puoi anche visualizzare l'utilizzo burst non fatturato (accumulato ma non ancora fatturato) e l'utilizzo burst provvisorio (accumulato nel periodo di fatturazione corrente e non ancora fatturato).
...
È inoltre possibile scaricare i dati in formato Excel, incluso lo stato di burst (fatturato, non fatturato o provvisorio) e la fatturazione per
